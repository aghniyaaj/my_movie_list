...
class ApiService {
  Future<List<Movie>> getTrendingMovies() async {
    final response = await http.get(Uri.parse('${AppConstants.tmdbBaseUrl}/trending/movie/week?api_key=${AppConstants.tmdbApiKey}&language=id-ID'));
  ...
  Future<List<Movie>> searchMovies(String query) async {
    final response = await http.get(Uri.parse('${AppConstants.tmdbBaseUrl}/search/movie?api_key=${AppConstants.tmdbApiKey}&language=id-ID&query=$query'));
    if (response.statusCode == 200) {
      final List results = json.decode(response.body)['results'];
      return results.map((e) => Movie.fromJson(e)).toList();
    }
  ...
  Future<List<Movie>> getMoviesByGenre(int genreId) async {
    final response = await http.get(Uri.parse('${AppConstants.tmdbBaseUrl}/discover/movie?api_key=${AppConstants.tmdbApiKey}&language=id-ID&with_genres=$genreId&sort_by=popularity.desc'));
    if (response.statusCode == 200) {
      final List results = json.decode(response.body)['results'];
      return results.map((e) => Movie.fromJson(e)).toList();
    }
