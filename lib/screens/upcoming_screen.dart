...
class _UpcomingScreenState extends State<UpcomingScreen> {
  ...
  void _fetchMovies() async {
    try {
      final movies = await ApiService().getUpcomingMovies(page: 1);
      if (mounted) {
        setState(() {
  ...
  void _fetchMoreMovies() async {
    ...
    _currentPage++;
    try {
      final movies = await ApiService().getUpcomingMovies(page: _currentPage);
      if (mounted) {
        setState(() {
          _movies.addAll(movies);
              ...
              : GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return GestureDetector(
                      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => DetailScreen(movie: movie, isUpcomingMovie: true))),
                      child: ClipRRect(
